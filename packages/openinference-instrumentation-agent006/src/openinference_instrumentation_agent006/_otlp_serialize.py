...
def value_to_otlp(value: object) -> dict:
    ...
    if isinstance(value, dict):
        return {"kvlistValue": {"values": [{"key": k, "value": value_to_otlp(v)} for k, v in value.items()]}}
...
def attrs_to_otlp(
    ...
    if exclude_prefixes:
        return [
            {"key": k, "value": value_to_otlp(v)} for k, v in attributes.items() if not k.startswith(exclude_prefixes)
        ]
    ...
    for _res_id, group in resource_groups.items():
        ...
        for scope_name, scope_group in scope_groups.items():
            ...
            scope_spans.append(
                {
                    "scope": scope_obj,
                    "spans": [span_to_otlp(s, exclude_attr_prefixes=exclude_attr_prefixes) for s in scope_group],
                }
            )
