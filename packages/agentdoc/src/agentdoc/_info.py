...
class FieldInfo:
    ...
    description: Annotated[str | None, "Description from annotation, docstring, or Pydantic Field"] = None
...
class ModuleInfo:
    ...
    classes: Annotated[list[tuple[str, str | None]], "Public classes as (name, one-liner docstring) pairs"] = field(
        default_factory=list
    )
    values: Annotated[list[tuple[str, str]], "Public non-callable values as (name, repr) pairs"] = field(
        default_factory=list
    )
    ordered_names: Annotated[list[str], "Symbol names in __all__ declaration order"] = field(default_factory=list)
    submodules: Annotated[list[tuple[str, str | None]], "Submodules as (qualified_name, one-liner docstring) pairs"] = (
