...
class _Hidden:
    ...
    def __enter__(self):
        ...
        if frame.f_locals is not frame.f_globals:
            raise RuntimeError("'with hidden:' must be used at module level, not inside a function or class body")
        module_name = frame.f_globals.get("__name__")
        module = sys.modules.get(module_name) if module_name else None
        if module is None:
            raise RuntimeError("hidden must be used at module level, could not determine calling module.")
...
def is_hidden_field(cls: Any, name: str) -> bool:
    ...
    for klass in cls.__mro__:
        ...
        hint = resolved.get(name) if resolved is not None else _resolve_single_annotation(klass, name)
        ...
        if origin is Annotated:
            ...
            from agentdoc._docs import SpecAnnotation

            return any(
                arg is hidden or (isinstance(arg, SpecAnnotation) and arg.kwargs.get("hidden") is True)
                for arg in args[1:]
            )
        return False
    return False
