...
from agentdoc.ext import CallableInfo, ModuleInfo
...
def _plotly_doc(mod) -> ModuleInfo:
    return ModuleInfo(
        ...
        submodules=[
            ...
            ("plotly.graph_objects", "Low-level API — Figure, Layout, and individual trace classes."),
            ("plotly.io", "Figure I/O: show(), write_html(), write_image(), read_json()."),
            ("plotly.colors", "Named colour sequences and scales (e.g. px.colors.qualitative.Plotly)."),
...
def _px_doc(mod) -> ModuleInfo:
    return ModuleInfo(
        ...
        functions=[
            ...
            # --- Part-of-whole ---
            CallableInfo(
                "pie", "(data_frame=None, names=None, values=None, color=None, hole=0, ...)", "Figure", "Pie chart."
            ),
            CallableInfo(
                "sunburst",
