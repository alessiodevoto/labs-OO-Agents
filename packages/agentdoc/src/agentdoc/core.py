...
def doc(
    *objs: Any,
    ...
    return _doc_multiple(flat_objs, concise=concise, inline_depth=inline_depth, config=DEFAULT_CONFIG)
...
def variables(
    ...
    if hasattr(obj, "__dict__"):
        for name, value in sorted(obj.__dict__.items()):
            ...
            line = f"{name}: {type_name} = {value_str}" if cfg.include_types else f"{name} = {value_str}"
