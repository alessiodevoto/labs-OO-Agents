...
from agentdoc._info import REQUIRED, CallableInfo, FieldInfo, ModuleInfo, TypeInfo
from agentdoc.protocols import SupportsCallableInfo, SupportsTypeInfo
    ...
    if isinstance(value, (list, tuple, dict, set, frozenset)) and len(value) <= 5:
        try:
            if isinstance(value, dict):
                ...
                all_primitive = all(isinstance(x, (str, int, float, bool, type(None))) for x in value)
            if all_primitive:
                return value
        except TypeError:
            pass
    ...
    if not _skip_protocol:
        ...
        if extractor:
            ...
            if result is not None:
                fields = (
                    result.fields if _include_hidden else [f for f in result.fields if not is_hidden_field(obj, f.name)]
                )
                result = TypeInfo(
                    name=result.name,
                    base=result.base,
...
def extract_callable_info(obj: Any) -> CallableInfo:
    ...
    try:
        ...
        docstring = _augment_docstring_with_annotated_descriptions(obj, sig, docstring, resolved_hints=resolved)
...
def extract_module_info(obj: Any) -> ModuleInfo:
    ...
    candidate_names = list(all_names) if all_names is not None else sorted(n for n in dir(obj) if not n.startswith("_"))
    ...
    if submodule_names and not isinstance(submodule_names, str) and hasattr(submodule_names, "__iter__"):
        import importlib
    ...
    return ModuleInfo(
        ...
        classes=classes,
        values=values,
        ordered_names=[n for n in candidate_names if n in resolved_names and n not in module_attr_names],
...
def _format_signature_params(
    ...
    for param_name, param in sig.parameters.items():
        ...
        if prev_kind == inspect.Parameter.POSITIONAL_ONLY and kind != inspect.Parameter.POSITIONAL_ONLY:
            params.append("/")
...
def _detect_base(obj: type) -> str | None:
    ...
    # NamedTuple
    if hasattr(obj, "_fields") and isinstance(getattr(obj, "_fields", None), tuple) and issubclass(obj, tuple):
        return "NamedTuple"
    ...
    for name, field_info in obj.model_fields.items():
        ...
        try:
            ...
            try:
                ...
                has_default = getattr(field_info, "default_factory", None) is not None or field_info.default is not None
        ...
        fields.append(FieldInfo(name=name, type=type_str, default=default, description=description, repr=repr_value))
    return fields
    ...
    for field in dataclasses.fields(obj):
        ...
        if field.default is not dataclasses.MISSING:
            ...
            try:
                ...
                default = f"{getattr(field.default_factory, '__name__', repr(field.default_factory))}()"
        else:
            default = REQUIRED

        fields.append(FieldInfo(name=field.name, type=type_str, default=default, description=description))
    return fields
...
def _extract_typeddict_fields(obj: type) -> list[FieldInfo]:
    ...
    for name, type_hint in annotations.items():
        ...
        if annotated_desc:
            ...
            description = None
        fields.append(FieldInfo(name=name, type=type_str, default=REQUIRED, description=description))
    return fields
...
def _extract_attrs_fields(obj: type) -> list[FieldInfo]:
    ...
    try:
        ...
        for attrib in attr.fields(obj):
            ...
            fields.append(FieldInfo(name=attrib.name, type=type_str, default=default, description=description))
    except ImportError:
...
def _has_docstring_section(docstring: str, section_name: str) -> bool:
    ...
    if section_name == "Args":
        ...
        return "returns:" in docstring_lower or ":return:" in docstring_lower or ":returns:" in docstring_lower
...
def _extract_plain_class_fields(obj: type) -> list[FieldInfo]:
    ...
    for name, _raw_hint in all_raw_annotations.items():
        ...
        if not description:
            from agentdoc._metadata import get_field_metadata
...
def _extract_cached_property_field(owner: type, name: str, cp: functools.cached_property) -> FieldInfo:  # type: ignore[type-arg]
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.AnnAssign):
            ...
            if isinstance(target, ast.Attribute) and isinstance(target.value, ast.Name) and target.value.id == "self":
                ...
                default = _ast_to_default(node.value, init_params) if node.value is not None else REQUIRED
...
def _extract_methods(obj: type) -> list[CallableInfo]:
    ...
    """
    from agentdoc._visibility import is_hidden_method
    ...
    from agentdoc._metadata import _FIELDS_ATTR  # noqa: PLC0415
