...
def make_llm_span(
    ...
    if tool_calls:
        for i, (func_name, args) in enumerate(tool_calls):
            flat_attrs[f"llm.output_messages.0.message.tool_calls.{i}.tool_call.function.name"] = func_name
            flat_attrs[f"llm.output_messages.0.message.tool_calls.{i}.tool_call.function.arguments"] = args
...
class TestTraceExplorerRealTrace:
    ...
    def real_trace_path(self):
        ...
        fixture_dir = workspace_root / "util/e2e_optimization/src/e2e_optimization/mechanical_checks/fixtures"
...
class TestAgentSpanParsing:
    ...
    def test_parses_real_validation_error_trace(self):
        ...
        assert len(llm_turns) > 0 or len(exec_turns) > 0, "Expected at least one LLM or execution turn"
...
class TestLoadSpansOtlp:
    ...
    def test_loads_otlp_jsonl(self):
        ...
        from trace_explorer.explorer import _load_spans
