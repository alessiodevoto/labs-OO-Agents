...
class TestConcurrentSpanTracking:
    ...
    async def test_concurrent_top_level_calls_all_tracked(self, mock_tracer, temp_trace_dir):
        ...
        assert all(ctx is not None for ctx in contexts_returned), "Some before_agent_call() returned None context"
...
class TestBeforeGenerationFallback:
    ...
    @pytest.mark.asyncio
    async def test_before_generation_does_not_steal_unrelated_span_as_parent(self, recording_tracer):
