...
class TuiConfigurationSkill(Skill):
    ...
    Layered (last wins): user ``~/.config/nooa/settings.yaml`` →
    project ``<project-root>/.nooa/settings.yaml`` →
    ...
    tui:
      ...
      trace_dir: .nooa/traces       # Trace output directory
      ...
      memory_agents:
        "nooa_tui.tui.agent:TUIAgent": "project"
      reflection: false                # idle-reflection default (/reflection on|off)
      reflection_agents:
        "nooa_tui.tui.agent:TUIAgent": true
    ...
    Structure of a skills repo::
        ...
        my-skills/
        ├── gitlab_ci/
        │   ├── pyproject.toml   # [project.entry-points."nooa.skills"]
    ...
    def _show_libs(self) -> str:
        ...
        if not libs_dirs:
            return (
                ...
                'entry-point under `[project.entry-points."nooa.skills"]`.\n\n'
        ...
        lines.append("")
        lines.append(
            "To add another directory: edit `.nooa/settings.yaml` and append to the `libs_dirs` list."
        )
