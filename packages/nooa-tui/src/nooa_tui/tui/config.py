...
Resolution order (last wins):
    ...
    2. Layered ``settings.yaml`` (user → project → ``NEMO_OO_SETTINGS``),
       loaded via :mod:`nooa_tui.tui.settings`
...
class Config(BaseModel):
    ...
    def load(cls, **overrides) -> "Config":
        ...
        try:
            ...
            for _ep in _entry_points(group="nooa_tui.skills_dirs"):
                try:
        ...
        # "nooa_tui.libs_dirs") — appended to libs_dirs so bootstrap's
        # discover_libs picks them up. Ships e.g. the inception lib.
        try:
            from importlib.metadata import entry_points as _entry_points

            for _ep in _entry_points(group="nooa_tui.libs_dirs"):
                try:
