...
Covers:
- nooa_cli._common
- nooa_cli.commands.traces
- nooa_cli.commands.delete_traces
- nooa_cli.completion
- nooa_cli.commands._otlp_helpers
"""
...
from nooa_cli._common import format_size

# ---------------------------------------------------------------------------
# _common.py
# ---------------------------------------------------------------------------
from nooa.paths import find_project_root
...
class TestCommonFindProjectRoot:
    ...
    def test_walks_up_from_cwd(self, tmp_path, monkeypatch):
        from nooa_cli._common import find_project_root as common_root
    ...
    def test_does_not_resolve_cli_package_dir(self, tmp_path, monkeypatch):
        ...
        assert "nooa_cli" not in str(result)


# ---------------------------------------------------------------------------
# _otlp_helpers.py
# ---------------------------------------------------------------------------
from nooa_cli.commands._otlp_helpers import (  # noqa: E402
...
# delete_traces.py
# ---------------------------------------------------------------------------
from nooa_cli.commands.delete_traces import _validate_endpoint  # noqa: E402
from nooa_cli.commands.delete_traces import command as delete_traces_command  # noqa: E402
...
# ---------------------------------------------------------------------------
from nooa_cli.commands.traces import (  # noqa: E402
    _collect_files,
...
class TestDiscoverTraceDirs:
    ...
    def test_finds_project_dir_traces(self, tmp_path):
        project_traces = tmp_path / ".nooa" / "traces"
    ...
    def test_default_root_uses_cli_project_root_for_project_trace_dir(self, tmp_path, monkeypatch):
        from nooa_cli.commands import traces as traces_mod
...
# completion.py
# ---------------------------------------------------------------------------
from nooa_cli.completion import (  # noqa: E402
    _BASH_SCRIPT,
    _FISH_SCRIPT,
    _ZSH_SCRIPT,
    _detect_shell,
...
class TestRenderScript:
    def test_bash_script_rendered(self):
        ...
        assert "_nooa_completion" in result
        assert "_NOOA_COMPLETE" in result
        assert "nooa" in result
...
class TestCompletionCommands:
    ...
    def test_bash_command(self):
        ...
        assert result.exit_code == 0
        assert "_nooa_completion" in result.output
        assert "_NOOA_COMPLETE" in result.output

    def test_zsh_command(self):
        result = self.runner.invoke(zsh)
        assert result.exit_code == 0
        assert "#compdef nooa" in result.output

    def test_fish_command(self):
        result = self.runner.invoke(fish)
        assert result.exit_code == 0
        assert "complete -c nooa" in result.output

    def test_install_unknown_shell(self):
        with patch("nooa_cli.completion._detect_shell", return_value=None):
    ...
    def test_install_bash(self, tmp_path):
        ...
        profile.write_text("")
        with (
            patch("nooa_cli.completion._detect_shell", return_value="bash"),
            patch("nooa_cli.completion.Path.home", return_value=tmp_path),
        ):
            result = self.runner.invoke(install)
        # Either success or file writing worked
        assert result.exit_code == 0 or "_NOOA_COMPLETE" in profile.read_text()

    def test_install_zsh(self, tmp_path):
        profile = tmp_path / ".zshrc"
        profile.write_text("")
        with (
            patch("nooa_cli.completion._detect_shell", return_value="zsh"),
            patch("nooa_cli.completion.Path.home", return_value=tmp_path),
        ):
            result = self.runner.invoke(install)
        assert result.exit_code == 0
    ...
    def test_install_line_invokes_declared_console_script(self, tmp_path):
        ...
        pyproject = Path(__file__).parents[3] / "nooa-cli" / "pyproject.toml"
        ...
        assert rc_line == f'eval "$(_NOOA_COMPLETE=bash_source {script_name})"'

    def test_install_removes_stale_pre_rename_line(self, tmp_path):
        """A pre-rename install wrote `_NEMO_COMPLETE=... nooa` (#322); once
        the `nooa` binary exists that line evals help text on every shell
        startup, so `completion install` must remove it."""
        stale = 'eval "$(_NEMO_COMPLETE=bash_source nooa)"'
        profile = tmp_path / ".bashrc"
        ...
        assert 'eval "$(_NOOA_COMPLETE=bash_source nooa)"' in content
        assert "# stuff" in content

    def test_install_already_installed(self, tmp_path):
        profile = tmp_path / ".bashrc"
        profile.write_text("eval $(_NOOA_COMPLETE=bash_source nooa)\n")
        with (
            patch("nooa_cli.completion._detect_shell", return_value="bash"),
            patch("nooa_cli.completion.Path.home", return_value=tmp_path),
        ):
    ...
    def test_install_fish(self, tmp_path):
        ...
        with (
            patch("nooa_cli.completion._detect_shell", return_value="fish"),
            patch("nooa_cli.completion.Path.home", return_value=tmp_path),
        ):
            result = self.runner.invoke(install)
        assert result.exit_code == 0
