...
from nooa_tui.tui.output import (
    ClearScreen,
    HelpOutput,
    TableOutput,
    TextOutput,
...
async def test_models_command_output(handler):
    ...
    with patch("nooa.unifiedllm.MODELS", {"provider1/model1": None, "provider2/model2": None}):
...
async def test_handler_command_exception_returns_error_not_raise(registry, mock_frontend):
    ...
    from nooa_tui.tui.commands import Command
...
def test_skill_with_invalid_yaml_argument_hint_is_discovered(
    ...
    The fix: add a line-by-line regex fallback (same as nooa._parse_frontmatter).
    """
...
async def test_session_list_excludes_empty_sessions(handler):
    ...
    from nooa_tui.tui.session_manager import SessionMeta
    ...
    with patch(
        "nooa_tui.tui.commands.SessionManager.list_sessions",
...
def test_refresh_skill_commands_removes_stale_entry(registry):
    ...
    """
    from nooa_tui.tui.commands import _UserSkill
