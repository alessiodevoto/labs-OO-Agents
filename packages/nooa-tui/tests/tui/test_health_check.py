...
import pytest
from nooa_tui.tui.health_check import (
    _PROBE_MAX_TOKENS,
    _classify_error,
    _detect_provider,
...
class TestClassifyError:
    ...
    def test_auth_401_env_not_set(self):
        ...
        with (
            patch(
                "nooa_tui.tui.health_check._get_expected_env_var",
    ...
    def test_model_not_found_suggests_config_creation(self):
        ...
        with patch("nooa_tui.tui.health_check._has_project_config", return_value=False):
            result = _classify_error(exc, llm)
    ...
    def test_model_not_found_suggests_edit_config(self):
        llm = _make_llm(model="fake-model")
        exc = Exception("The model `fake-model` does not exist")
        with patch("nooa_tui.tui.health_check._has_project_config", return_value=True):
            result = _classify_error(exc, llm)
    ...
    def test_connection_with_yaml_mentions_api_base(self):
        llm = _make_llm(model="my-model")
        exc = ConnectionError("Connection refused")
        with patch("nooa_tui.tui.health_check._has_llm_config_yaml", return_value=True):
            result = _classify_error(exc, llm)
    ...
    def test_connection_without_yaml_mentions_provider_down(self):
        llm = _make_llm(model="my-model")
        exc = ConnectionError("Connection refused")
        with patch("nooa_tui.tui.health_check._has_llm_config_yaml", return_value=False):
            result = _classify_error(exc, llm)
...
class TestProbeLLM:
    ...
    async def test_timeout_probe(self):
        ...
        with patch("nooa_tui.tui.health_check._PROBE_TIMEOUT_SECONDS", 0.1):
            result = await probe_llm(llm)
