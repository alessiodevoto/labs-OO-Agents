...
async def test_bootstrap_project_memory_uses_working_dir_store(tmp_path, monkeypatch):
    ...
    result = await bootstrap(cfg)
    try:
        expected = (work_dir / ".nemo_oo" / "memory" / "memory.sqlite").resolve()
        mgr = result.agent.memory._mgr
...
def test_delete_session_never_touches_project_memory_db(tmp_path, monkeypatch):
    ...
    project_db = tmp_path / "work" / ".nemo_oo" / "memory" / "memory.sqlite"
...
async def test_memory_command_on_off_persists_to_settings_yaml_and_reloads(tmp_path, monkeypatch):
    ...
    try:
        ...
        expected = (Path(cfg.agent.working_dir) / ".nemo_oo" / "memory" / "memory.sqlite").resolve()
...
async def test_owner_resolution_override_chain(tmp_path, monkeypatch):
    ...
    try:
        ...
        cfg.tui.memory_owner_agents["nemo_oo_agents_cli.tui.agent:TUIAgent"] = "planner"
        from nemo_oo_agents_cli.tui.bootstrap import configure_tui_memory
...
async def test_legacy_owner_rows_are_healed_on_configure(tmp_path, monkeypatch):
    ...
    from nemo_oo_agents.memory import Memory
    ...
    work_dir = tmp_path / "work"
    work_dir.mkdir()
    legacy_key = "nemo_oo_agents_cli.tui.agent:TUIAgent"
    store_path = work_dir / ".nemo_oo" / "memory" / "memory.sqlite"
