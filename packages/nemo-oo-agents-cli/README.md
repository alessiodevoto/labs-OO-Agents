...
CLI and TUI for [nemo-oo-agents](https://gitlab-master.nvidia.com/interactive-agents/nemo_oo_agents). Ships the `nemo` command (subcommands like `nemo oo tui`, `nemo oo term`, `nemo oo eval`, `nemo oo trace`) and the agent TUI (a REPL-driven CodeAct frontend).
