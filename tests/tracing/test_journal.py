...
class TestHashMsg:
    ...
    def test_null_content_differs_from_empty_string(self):
        assert _hash_msg({"role": "assistant", "content": None}) != _hash_msg(
...
class TestContextBlockSideband:
    ...
    def test_system_message_expanded_into_blocks(self, cb):
        ...
        with patch(
            ...
            from nemo_oo_agents.tracing._context_sideband import set_context_blocks
    ...
    def test_changed_block_sends_only_delta(self, cb):
        ...
        with patch("nemo_oo_agents.tracing._litellm_journal._post_json", side_effect=record):
            from nemo_oo_agents.tracing._context_sideband import set_context_blocks

            # Turn 1: both blocks fresh
            set_context_blocks([block_a, block_b])
            cb._send_new_messages(
                "sess2", [{"role": "system", "content": f"{block_a}\n\n{block_b}"}]
            )
            posted.clear()
    ...
    def test_no_sideband_falls_back_to_whole_message(self, cb):
        ...
        from nemo_oo_agents.tracing._context_sideband import _current_blocks
    ...
    def test_block_sub_entries_sent_before_compound(self, cb):
        ...
        compound_h = _hash_msg(
            {"role": "system", "_blocks": [_hash_msg({"_block": b}) for b in blocks]}
        )
    ...
    def test_sideband_cleared_after_read_even_when_first_msg_not_system(self, cb):
        ...
        with patch("nemo_oo_agents.tracing._litellm_journal._post_json", return_value=True):
...
class TestSpanIdCapture:
    def test_span_id_captured_from_active_otel_span(self):
        ...
        with patch(
            "nemo_oo_agents.tracing._litellm_journal.otel_trace.get_current_span",
...
class TestJournalCallbackHooks:
    ...
    def test_log_success_event_without_prior_pre_api_call_logs_warning(self, cb):
        ...
        with (
            ...
            patch("nemo_oo_agents.tracing._litellm_journal.log") as mock_log,
        ):
            cb.log_success_event(
...
class TestSpanLimits:
    def test_tracer_provider_has_raised_span_attribute_limit(self):
        ...
        with (
            patch("nemo_oo_agents.tracing.NemoOOAgentsInstrumentor") as mock_cls,
            patch("nemo_oo_agents.tracing._instrument_litellm"),
            patch("nemo_oo_agents.tracing.trace.get_tracer_provider", return_value=None),
        ):
