...
3. **Set the unifiedllm config** so model aliases resolve:
   ```bash
   export UNIFIEDLLM_CONFIG="$(pwd)/configs/llm_config_nvidia.yaml"
   ```
...
- **`ValueError: Model 'X' not found.`** — make sure `UNIFIEDLLM_CONFIG` is set and points at `configs/llm_config_nvidia.yaml`.
...
1. Register the alias in `configs/llm_config_nvidia.yaml`:
   ```yaml
   my-new-model:
