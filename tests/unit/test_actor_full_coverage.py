...
# ===========================================================================
# Lines 1957-1958: _generated_source on dynamically defined plan method
# ===========================================================================


class TestDefineMethodGeneratedSource:
    """Lines 1955-1958: define_method sets _generated_source on the function.

    Line 1956 is the success path. Lines 1957-1958 catch exceptions when
    _generated_source assignment fails. We test both paths.
    """

    @pytest.mark.asyncio
    async def test_define_method_sets_generated_source(self):
        """define_method creates a method with _generated_source attached (line 1956)."""
        ...
        agent = _Agent()
        await agent.runtime.define_method(
            name="helper",
            params=["x: int"],
            return_type="int",
            docstring="Double x",
        ...
        assert hasattr(agent, "helper")
        # The bound method's underlying function should have _generated_source
        func = agent.helper.__func__
        assert hasattr(func, "_generated_source")
        assert "def helper" in func._generated_source

    @pytest.mark.asyncio
    async def test_define_method_generated_source_failure(self):
        """define_method silently ignores _generated_source failure (lines 1957-1958).

        We replace the function in the exec namespace with a frozen callable
        (using __slots__) that refuses _generated_source attribute assignment.
        """
        ...
        # Strategy: intercept the exec() call that define_method uses to
        # create the function. Replace the result with a callable that
        # has all required slots but not _generated_source.
        class FrozenFunc:
            """Callable that accepts decorator attrs but not _generated_source."""

            __slots__ = ("_agent_decorator", "_plan_strategy", "_needs_generation", "_name", "_doc")

            def __init__(self, name, doc):
                self._name = name
                self._doc = doc
                self._agent_decorator = None
        ...
        def _exec_that_freezes(code_obj, namespace, *args, **kw):
            _real_exec(code_obj, namespace, *args, **kw)
            # After exec creates the function, replace it with our frozen version
            for key in list(namespace.keys()):
                if key.startswith("_"):
                    continue
                val = namespace[key]
                if callable(val) and hasattr(val, "__name__"):
                    frozen = FrozenFunc(val.__name__, val.__doc__)
                    namespace[key] = frozen

        with patch("builtins.exec", side_effect=_exec_that_freezes):
            # define_method internally does:
            # 1. exec(func_code, namespace) -> creates func
            # 2. func._agent_decorator = "plan"  (works on FrozenFunc)
            # 3. func._plan_strategy = strat (works)
            # 4. func._needs_generation = needs_generation (works)
            # 5. func._generated_source = func_code -> FAILS on FrozenFunc (line 1957-1958)
            # 6. types.MethodType(func, agent) -> FAILS because FrozenFunc is not a function
            # We need types.MethodType to work, so let's patch that too.
            with patch("nemo_oo_agents.runtime.actor.types.MethodType", side_effect=lambda f, a: f):
                await agent.runtime.define_method(
                    name="frozen_helper",
                    params=["x: int"],
                    return_type="int",
        ...
        # Method should still be defined despite _generated_source failure
        assert hasattr(agent, "frozen_helper")
