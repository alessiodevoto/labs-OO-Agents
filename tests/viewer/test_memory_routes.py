...
from nemo_oo_agents.viewer import memory_routes
...
def test_dbs_discovery(client, tmp_path, monkeypatch):
    for rel in (".nemo_oo/memory/project.sqlite", ".nemo_oo/sessions/abc-memory.db"):
        MemoryStore(tmp_path / rel).close()  # MemoryStore mkdir-s parents itself
    # A non-memory session db must NOT be discovered.
    (tmp_path / ".nemo_oo/sessions/abc.db").write_bytes(b"")
    monkeypatch.chdir(tmp_path)
    ...
    assert str(tmp_path / ".nemo_oo/memory/project.sqlite") in paths
    assert str(tmp_path / ".nemo_oo/sessions/abc-memory.db") in paths
    assert str(tmp_path / ".nemo_oo/sessions/abc.db") not in paths
