...
**Summary:** Use the same `Annotated[T, hidden]` marker for types and agents. doc() and structured output can be implemented with a small agentdoc extension (field filter) and changes in nemo_oo_agents; code blocking for non-agent types is a follow-up (type tracking or conservative name set).
