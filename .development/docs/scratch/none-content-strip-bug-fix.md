...
2. **nemo_oo_agents** (`src/nemo_oo_agents/strategies/structured_output.py`): `_strip_xml_wrapper(content)` did `content.strip()` without handling `None`, so any code path that passed `None` (e.g. from `assistant_message["content"]` or future callers) raised `AttributeError`.

## Changes made

- **unifiedllm**: Added `or ""` when setting `assistant_message["content"]` in both sync and async tool_calls branches (lines 792 and 903).
- **nemo_oo_agents**: In `_strip_xml_wrapper`, use `(content or "").strip()` and type hint `content: str | None`.
- **Tests**:
