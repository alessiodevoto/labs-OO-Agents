...
Issue: https://gitlab-master.nvidia.com/interactive-agents/nooa/-/issues/192
...
`@hidden` only suppresses the method from `doc(self)` / LLM API view; it does **not** disable tracing. Per CLAUDE.md (and `_should_trace` in `src/nooa/metaclass.py:132-155`), private and dunder methods are traced by default when `_enable_tracing = True` unless explicitly marked `@no_trace`.

## Fix (targeted, smallest blast radius)

Decorate every non-generation `@hidden` helper in `src/nooa/agents/summarization.py` with `@no_trace`. Leave `summarize()` (the only true LLM call) traced.
...
1. In `src/nooa/agents/summarization.py`:
   - Add `from nooa.metaclass import no_trace` (matches the existing per-module import style — `Agent` from `nooa.agent`, `hidden` from `nooa.agentdoc`).
...
2. Add a regression test in `tests/agents/test_summarization_agents.py` (or a new `tests/agents/test_summarization_tracing.py` if cleaner) that:
   ...
   - Use `is_hidden_method` from `nooa.agentdoc._visibility` to detect `@hidden`.
...
- `uv run ruff check src/nooa/agents/summarization.py tests/agents/`
