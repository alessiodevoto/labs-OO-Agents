...
``nemo_oo_agents.memory``: record listing + detail (with the per-memory Usage
...
from nemo_oo_agents.memory.config import EmbeddingConfig, ForgetPolicy, RetrievalConfig
from nemo_oo_agents.memory.embeddings import get_embedder
from nemo_oo_agents.memory.forgetting import ForgettingEngine
from nemo_oo_agents.memory.observability import per_memory_usage, store_kpis
from nemo_oo_agents.memory.retrieval import RetrievalEngine
from nemo_oo_agents.memory.schema import Memory
from nemo_oo_agents.memory.store import MemorySchemaError, MemoryStore
...
def list_dbs() -> list[dict]:
    ...
    Scans ``<cwd>/.nemo_oo/memory/*.sqlite`` (project stores) and
    ``<cwd>/.nemo_oo/sessions/*-memory.db`` (per-session TUI stores). This is
