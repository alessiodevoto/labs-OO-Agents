# `nemo_oo_agents.memory` — long-term memory for agents
...
A nemo-oo agent is normally amnesic between method calls and sessions. This add-on
...
```python
from nemo_oo_agents.memory import MemoryConfig, MemoryManager
from nemo_oo_agents.memory.config import SpontaneousConfig
...
- **logger** `nemo_oo_agents.memory` — `setLevel(logging.DEBUG)` for per-op traces.
...
```python
import logging; logging.getLogger("nemo_oo_agents.memory").setLevel(logging.DEBUG)
