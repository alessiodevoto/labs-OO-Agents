...
"""ShellTools5 — ShellTools4 + transparent grep->Match.
...
Attach to an agent::
    ...
    class MyAgent(Agent, llm=llm):
        def __init__(self, *a, **kw):
            super().__init__(*a, **kw)
            self.shell = ShellTools5(cwd="/path/to/repo")
"""
...
class ShellTools5(Skill):
    """
    Persistent shell + file ops, with grep that hands you editable Match objects.

    Four methods (same surface as ShellTools4 — no new tools to learn):
