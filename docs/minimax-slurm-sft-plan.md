...
- `dabstep_sft_nooa_minimax_m2.5_YYYYMMDD.jsonl` — opt63 (nooa variety)
...
python3 -c "
import json
f='experiments/evaluation-ablations/results/dabstep/minimax_m2.5_opt63_$(date +%Y%m%d)/nooa_dabstep_full.noo-eval.jsonl'
...
python3 -c "
import json
f='experiments/evaluation-ablations/results/dabstep/minimax_m2.5_agent009_$(date +%Y%m%d)/nooa_dabstep_full.noo-eval.jsonl'
results = [json.loads(l) for l in open(f) if json.loads(l).get('_type')=='result']
...
# Output: sft_data/dabstep_agent009/dabstep_sft_nooa_minimax_m2.5_YYYYMMDD.jsonl(.gz)
