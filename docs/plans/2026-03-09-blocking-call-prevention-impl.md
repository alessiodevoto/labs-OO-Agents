...
**Files:**
- Create: `src/nooa/runtime/restrictions.py`
...
```python
# src/nooa/runtime/restrictions.py
...
```bash
git add src/nooa/runtime/restrictions.py tests/runtime/test_restrictions.py
...
**Files:**
- Modify: `src/nooa/config/strategy_config.py:6-26`
...
from nooa.runtime.restrictions import DEFAULT_BLOCKED_CALLS, DEFAULT_BLOCKED_MODULES
...
In `src/nooa/config/strategy_config.py`, add two fields to `CodeActConfig`:
...
```bash
git add src/nooa/config/strategy_config.py tests/config/test_blocked_config.py
...
**Files:**
- Modify: `src/nooa/runtime/code_validator.py:62-71`
- Modify: `src/nooa/runtime/actor.py:709-718`
...
In `src/nooa/runtime/code_validator.py`, add field to `ValidationContext` (around line 71):
...
In `src/nooa/runtime/actor.py`, pass exec_globals when constructing ValidationContext (around line 709):
...
```bash
git add src/nooa/runtime/code_validator.py src/nooa/runtime/actor.py tests/runtime/test_validation_context.py
...
**Files:**
- Modify: `src/nooa/runtime/code_validator.py` — add new class, replace AsyncSafetyValidator
...
In `src/nooa/runtime/code_validator.py`, add the new class. Replace `AsyncSafetyValidator` in `UnifiedCodeValidator.__init__`.
...
```bash
git add src/nooa/runtime/code_validator.py tests/runtime/test_blocking_call_validator.py
...
**Files:**
- Modify: `src/nooa/runtime/actor.py:720-721`
- Modify: `src/nooa/strategies/codeact.py` (where execute_code is called)

**Step 1: Trace how cell_timeout flows from config to execute_code**

Read `src/nooa/strategies/codeact.py:1666` to understand how `self.config.cell_timeout` is passed. Follow the same pattern for `blocked_modules`/`blocked_calls`.
...
from nooa.runtime.code_validator import UnifiedCodeValidator, ValidationContext, ValidationError
...
```bash
git add src/nooa/runtime/actor.py src/nooa/strategies/codeact.py tests/runtime/test_config_wiring.py
...
**Files:**
- Modify: `src/nooa/runtime/actor.py:628-670` (exec_globals construction)
...
In `src/nooa/runtime/actor.py`, add a function and call it during exec_globals construction:
...
```bash
git add src/nooa/runtime/actor.py tests/runtime/test_exec_globals_stripping.py
...
**Files:**
- Modify: `src/nooa/runtime/code_validator.py` — remove `AsyncSafetyValidator` and `_AsyncSafetyVisitor`
...
```bash
git add src/nooa/runtime/code_validator.py tests/runtime/test_code_validator.py tests/runtime/test_async_deadlock_prevention.py
...
**Files:**
- Modify: `src/nooa/runtime/__init__.py` (if restrictions.py needs exporting)
