...
class Evaluator:
    ...
    def build_samples_for_pairs(
        self,
        pairs: list[tuple[str, str]],
        runs: int = 1,
    ) -> list[Sample]:
        """Build samples for specific (task_id, model_id) pairs.

        This enables sparse evaluation - running only specific combinations
        instead of the full Cartesian product.

        Args:
            pairs: List of (task_id, model_id) tuples
            runs: Number of runs per pair

        Returns:
            List of Sample objects ready to run
        """
        samples = []

        # Build task lookup: task_id -> (test, task, task_index)
        task_lookup = {}
        for test in self.tests.values():
            for i, task in enumerate(test.data):
                task_lookup[task.id] = (test, task, i)

        # Build samples for each pair
        for task_id, model_id in pairs:
            if task_id not in task_lookup:
                raise ValueError(f"Task '{task_id}' not found in test suite")
            if model_id not in self.models:
                raise ValueError(
                    f"Model '{model_id}' not found. Available: {list(self.models.keys())}"
                )

            test, task, task_index = task_lookup[task_id]
            client = self.models[model_id]

            for run_id in range(1, runs + 1):
                sample = self._create_sample(test, task, task_index, client, model_id, run_id)
    ...
    async def run_samples(
        ...
        Args:
            samples: List of Sample objects to run
            parallel: Max concurrent samples
            quiet: Minimal output
        ...
        # Setup output directory (microseconds for uniqueness when run multiple times)
        ts = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        run_experiment_name = f"{self.name}_{ts}"
        _tmpdir = tempfile.TemporaryDirectory() if no_files else None
        experiment_dir = Path(_tmpdir.name) if no_files else self.output_dir / run_experiment_name
        if not no_files:
            experiment_dir.mkdir(parents=True, exist_ok=True)
        ...
        model_ids = list({s.model for s in samples})
        test_names = list({s.display_name.split("/")[0] for s in samples})
        ...
        writer.start(
            ...
            models=[self._model_metadata.get(m, {"id": m}) for m in model_ids],
            tests=test_names,
            runs=1,  # Samples already include run info
        )
        ...
        try:
            all_results = await run_evaluation(
                samples, pipeline_config, writer, max_concurrent=parallel
            )
        finally:
            try:
                writer.finalize()
